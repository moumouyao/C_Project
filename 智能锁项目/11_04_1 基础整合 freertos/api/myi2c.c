...
/**
  ...
  */
	void myi2c_w_scl(uint8_t bitval){
		GPIO_WriteBit(GPIOA, GPIO_Pin_8, (BitAction)bitval);
		Delay_us(10);
	}
  ...
  */
	void myi2c_w_sda(uint8_t bitval){
		GPIO_WriteBit(GPIOC, GPIO_Pin_9, (BitAction)bitval);
		Delay_us(10);
	}
...
void myi2c_init(void){
	//初始化GPIO口
  	...
  	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;	//时钟线
  	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  	GPIO_Init(GPIOA, &GPIO_InitStructure);
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9;	//数据线
