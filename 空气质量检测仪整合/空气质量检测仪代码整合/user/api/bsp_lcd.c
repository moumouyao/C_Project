  ...
  u8 temp,t1,t;
	...
	{   
		...
		{			    
			...
			else if(mode==0){LCD_Fast_DrawPoint(x,y,BACK_COLOR);}//LCD_DrawPoint(x,y);//LCD_Fast_DrawPoint(x,y,BACK_COLOR);
			temp<<=1;
			y++;
...
void drawline(u32 a,u32 b,u32 c,u32 d)
{
	...
	{	
       switch(flag&0x07)
	  {
       	case 0:LCD_DrawPoint(a+i,b+j); break; //画点时要将坐标平移回来
				case 1:LCD_DrawPoint(a-i,b+j); break;
				case 2:LCD_DrawPoint(a+i,b-j); break;
				case 3:LCD_DrawPoint(a-i,b-j); break;
				case 4:LCD_DrawPoint(a+j,b+i); break;
				case 5:LCD_DrawPoint(a-j,b+i); break;
				case 6:LCD_DrawPoint(a+j,b-i); break;
				case 7:LCD_DrawPoint(a-j,b-i); break;
				default:break;
	  }			 		
...
void draw_circle(u32 a,u32 b,u32 r)
{
	...
	while((i+j!=r)&&(i+j!=r+1))
	{	
		LCD_DrawPoint(a+i,b+r-j);
		LCD_DrawPoint(a+i,b-r+j);
		LCD_DrawPoint(a-i,b+r-j);
		LCD_DrawPoint(a-i,b-r+j);
		
		LCD_DrawPoint(a+r-j,b+i);
		LCD_DrawPoint(a-r+j,b+i);
		LCD_DrawPoint(a+r-j,b-i);
		LCD_DrawPoint(a-r+j,b-i);
...
void show_chin2LCD(uint8_t x, uint8_t y, uint8_t *str){
	uint16_t cnttt = 0;
	//比如说我要在 左上角(0,0)显示 它，逐个点的去描绘就可以了
	for(uint8_t i=0;i<32;i++) {
		for(uint8_t j=0;j<8;j++) {
			if((str[i] & (1<<(7-j))) != 0) {
				LCD_DrawPoint(cnttt%16 + y, cnttt/16 + x);
			}
			cnttt++;	
		}
	}
...
void show_char2LCD(uint8_t x, uint8_t y, uint8_t *str){
	...
	for(uint8_t i=0;i<16;i++) {
		for(uint8_t j=0;j<8;j++) {
			if((str[i] & (1<<(7-j))) != 0) {
				LCD_DrawPoint(cnttt%8 + y, cnttt/8 + x);
			}
			cnttt++;	
		}
	}
