...
#define MEM_SIZE                (50*1024*3) 	//内存堆heap大小
#define MEMP_NUM_PBUF           5000 		//MEMP_NUM_PBUF:memp结构的pbuf数量,如果应用从ROM或者静态存储区发送大量数据时,这个值应该设置大一点 50
#define MEMP_NUM_UDP_PCB        20		//MEMP_NUM_UDP_PCB:UDP协议控制块(PCB)数量.每个活动的UDP"连接"需要一个PCB. 5
#define MEMP_NUM_TCP_PCB        20		//MEMP_NUM_TCP_PCB:同时建立激活的TCP数量 6
#define MEMP_NUM_TCP_PCB_LISTEN 20		//MEMP_NUM_TCP_PCB_LISTEN:能够监听的TCP连接数量6
#define MEMP_NUM_TCP_SEG        15		//MEMP_NUM_TCP_SEG:最多同时在队列中的TCP段数量
#define MEMP_NUM_SYS_TIMEOUT    20		//MEMP_NUM_SYS_TIMEOUT:能够同时激活的timeout个数 8

//pbuf选项
#define PBUF_POOL_SIZE          60		//PBUF_POOL_SIZE:pbuf内存池个数 20
#define PBUF_POOL_BUFSIZE       3072		//PBUF_POOL_BUFSIZE:每个pbuf内存池大小
...
//有关系统的选项
#define TCPIP_THREAD_STACKSIZE          2000	//内核任务堆栈大小 1000
