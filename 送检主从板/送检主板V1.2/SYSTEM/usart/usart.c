...
{
	UsartTranCnt = 0;                      //发送数据计数器清零
	UsartTranLength = num;                 //发送数据长度清零
	UsartTranDataPot = p;	                 //发送数据指针
...
//	USART_ClearFlag(USART3, USART_FLAG_TC);
  USART_ITConfig(USART3, USART_IT_RXNE, ENABLE);//开启接受中断
	USART_ITConfig(USART3, USART_IT_IDLE, ENABLE);//开启接受中断
  
  
  USART_SetAddress(USART3, addr);//应该是参数传错了
  /* Select the USART2 WakeUp Method */
  USART_WakeUpConfig(USART3, USART_WakeUp_AddressMark);	   //开启多处理器通讯
  /* Enable the USART2 mute mode*/
  USART_ReceiverWakeUpCmd(USART3, DISABLE);    //进入静默模式
